...
int main (int argc, char **argv)
{
    ...
    if (VMI_FILE != vmi_get_access_mode(vmi)) {
        ...
        printf("Process and DTB addresses for VM %s (id=%"PRIu64")\n", name2, id);
    }
    else {
	...
	printf("\nProcess Id:         Process Name    Directory Table Base Address \n");
    printf("---------------------------------------------------------------------------------------------------\n");
    ...
    do {
        ...
        addr_t dtb_addr = vmi_pid_to_dtb(vmi,pid);
        printf("[%5d] \t %20s \t %10"PRIx64"\n", pid, procname,dtb_addr);
        if (procname) {
            free(procname);
            procname = NULL;
        }
		...
		//printf("[%5d] %s (dtb address:%"PRIx64")\n", pid, procname,dtb_addr);
